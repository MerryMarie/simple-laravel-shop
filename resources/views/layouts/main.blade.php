...
<header class="bg-dark py-2 py-md-3">
    <div class="container d-flex flex-column flex-md-row align-items-center justify-content-between gap-2 gap-md-1">
        <a class="text-light text-decoration-none fs-4" href="{{route('home')}}">НА ГЛАВНУЮ</a>
        ...
        @else
            ...
            <span class="bg-primary fs-6 text-light px-2 rounded-pill">@price(Auth::user()->bonus_balls) баллов</span>
            <span class="login text-light fs-6"> {{ Auth::user()->email }}</span>
        </div>
        @endguest
        <div>
            <a class="btn position-relative" href="{{route('cart')}}" title="Корзина">
                ...
                @guest
                @else
                    <span data-cart_count class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                    {{$cart_count}}
                  </span>
                @endguest
            </a>
            ...
            <a class="btn" href="{{route('login')}}" title="Войти" data-login_btn>
                <svg width="35px" height="35px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            @else
            <button class="btn " title="Выйти" onclick="event.preventDefault();
